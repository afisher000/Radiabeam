classdef IFW
    ...
    methods
        function obj = IFW(camera)
            % IFW Class constructor.  

            % Read camera properties, make serial connection (s) to filter
            % wheel
            obj.device_ID   = camera.ID;
            obj.COM         = camera.filter_port;      
            obj.SERIALN     = camera.SN;
            fprintf('Connecting to filter wheel:\n');
            fprintf('\tDevice_ID = %d\n', obj.device_ID);
            fprintf('\tPort = %s\n', obj.COM);

            obj.s = serialport(obj.COM, obj.b_rate,"Timeout", obj.timeout);

            % Test communication. Read filter_ID and home filter wheel
