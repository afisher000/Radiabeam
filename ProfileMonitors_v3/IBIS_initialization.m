...
% This m-file sets up the GUI items, such as buttons, etc.
    ...
    h.rootdir = 'C:\Users\Marcos\Desktop\GRIT PM images'; % This is the root directory for saving image
    date_string = datestr(now, 'yyyy-mmmm-dd');

    fprintf('Day: %s\n', date_string);

    %% Create Directories
    h.dest_folder = fullfile(h.rootdir, date_string, h.Tag);
    h.dest_folder_color = fullfile(h.rootdir, date_string, h.Tag, 'Color');

    if ~exist(h.dest_folder, 'dir')
       mkdir(h.dest_folder);
    end
    if ~exist(h.dest_folder_color, 'dir')
        mkdir(h.dest_folder_color);
    end
    
    %% Connect to filter-wheel
    try
        objIFW = IFW(h.camera.ID, h.camera.filter_port);
