...
% during the hardware tringgering
    ...
    h.camera      = camera;
    h.name        = append('IBIS Viewer. ', camera.Tag);
    h.pixelSize   = camera.pixelCalibration;
    h.divisor     = 2;
    h.flip_v      = camera.flip_v;
    h.flip_h      = camera.flip_h;
    h.Tag         = camera.Tag;
...
%         stoppreview(vid)        
        gui.Enable  = 'on';   % allow button interaction
        gui.Value   = gui.Min;    %set the button to high
        stop(h.vid)
        delete(h.vid)
        clear h.vid
        delete(h.figure)
